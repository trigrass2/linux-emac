...
int atomic_pwm_widthcount_write(pwm_t * pwm, pwm_data data)
{
	ATOMIC(pwm->widthcount_write(pwm, data);
	    )
	    return 0;
}
...
pwm_data atomic_pwm_widthcount_read(pwm_t * pwm)
{
	pwm_data retval;
	ATOMIC(retval = pwm->widthcount_read(pwm);
	    )
	    return retval;
}
...
{
	size_t size;
	pwm_t *pwm = dev_get_drvdata(cls);
	pwm_data data = sfs_getint(buf, count, &size);
	atomic_pwm_widthus_write(pwm, data);
	return size;
}

static ssize_t pwm_widthcount_store(struct device *cls,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	size_t size;
	pwm_t *pwm = dev_get_drvdata(cls);
	pwm_data data = sfs_getint(buf, count, &size);
	atomic_pwm_widthcount_write(pwm, data);
	return size;
}
...
static ssize_t pwm_widthcount_show(struct device *cls,
				struct device_attribute *attr, char *buf)
{
	pwm_t *pwm = dev_get_drvdata(cls);
	return sprintf(buf, "%u\r\n", atomic_pwm_widthcount_read(pwm));
}
...
static DEVICE_ATTR(widthus, S_IRUGO | S_IWUGO, pwm_widthus_show,
		   pwm_widthus_store);
static DEVICE_ATTR(widthcount, S_IRUGO | S_IWUGO, pwm_widthcount_show,
		   pwm_widthcount_store);
	...
	if ((pwm->widthus_write) && (pwm->widthus_read))
		if (device_create_file(dev, &dev_attr_widthus))
			printk(KERN_ERR "Error creating sysfs widthus for %s\n",
					pwm->name);
	if ((pwm->widthcount_write) && (pwm->widthcount_read))
		if (device_create_file(dev, &dev_attr_widthcount))
			printk(KERN_ERR "Error creating sysfs widthcount for %s\n",
					pwm->name);
