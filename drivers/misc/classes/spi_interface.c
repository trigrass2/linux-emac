...
/**
 * Provides an interface between the Linux SPI interface and the
...
EXPORT_SYMBOL(lsi2esc_spi_xmit);
...
EXPORT_SYMBOL(lsi2esc_spi_confread);
...
EXPORT_SYMBOL(lsi2esc_spi_confwrite);
...
EXPORT_SYMBOL(lsi2esc_spi_speedread);
...
EXPORT_SYMBOL(lsi2esc_spi_speedwrite);
...
static int lsi2esc_probe(struct spi_device *spi)
{
	struct spi_s *esc;
	struct device_node *np;
	/* the platform data of dev must be set to the spi_t in
	 * the board specific file */
	/* creates a loop so that objects may refer to one another */
	...
	if(np && !esc) {
		esc = kmalloc(sizeof(struct spi_s), GFP_KERNEL);
		if(!esc)
		       return -ENOMEM;	
		of_property_read_string(np, "device-name", &esc->name); 
		of_property_read_string(np, "gpio-name", &esc->gpio_name); 
		esc->subclass = 0;
		esc->tip = lsi2esc_spi_tip;
		esc->xmit = lsi2esc_spi_xmit;
		esc->confwrite = lsi2esc_spi_confwrite;
		esc->confread = lsi2esc_spi_confread; 
		esc->speedread = lsi2esc_spi_speedread;
		esc->speedwrite = lsi2esc_spi_speedwrite;
		esc->gpio_create = lsi2esc_find_gpio_create(esc->gpio_name);
		esc->gpio_data = NULL;
	}
#endif
	...
	if (!spi_class_register_device(esc))
		return -ENOMEM;
...
static int lsi2esc_remove(struct spi_device *spi)
{
	struct spi_s *esc = spi->dev.platform_data;

	spi_class_unregister_device(esc);
	return 0;
}

#if defined(CONFIG_OF)
static const struct of_device_id lsi2esc_dt_ids[] = {
	{ .compatible = "lsi2esc" },
	{ /* sentinel */ },
};

MODULE_DEVICE_TABLE(of, lsi2esc_dt_ids);
#endif
...
static struct spi_driver lsi2esc_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
#if defined(CONFIG_OF)
		.of_match_table = of_match_ptr(lsi2esc_dt_ids),
#endif
	},
	.probe = lsi2esc_probe,
	.remove = lsi2esc_remove,
};
